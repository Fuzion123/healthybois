...
function ParticipantBar({ eventId }) {
    ...
    if (data) {
      setParticipantPositions((prevPositions) => {
        ...
        data.forEach((participant, index) => {
          const existingIndex = prevPositions.findIndex((pos) => pos && pos.id === participant.id);
          if (existingIndex !== -1) {
  ...
  const handleDrop = (e, pointIndex) => {
    ...
    if (draggedParticipant !== null) {
      setParticipantPositions((prevPositions) => {
        ...
        const oldPositionIndex = prevPositions.findIndex((pos) => pos === draggedParticipant);
  ...
  <div
    key={p.id}
    className={`relative flex-shrink-0 w-16 h-16 ${participantPositions[index] ? "opacity-0" : ""} rounded-full overflow-hidden bg-gray-200`}
    draggable="true"
