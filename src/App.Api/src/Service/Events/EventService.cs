...
namespace Service.Events
{
		public class EventService
		{
				...
				public async Task<ActivityListingDto> AddOrUpdateResults(int eventId, int activityId, List<ResultInput> inputs, CancellationToken cancellationToken)
				{
						var @event = await eventRepository.GetById(eventId, cancellationToken);
						...
						if (@event.AddOrUpdateActivityResults(activityId, inputs))
						{
								await eventRepository.SaveChangesAsync(cancellationToken);
						}
